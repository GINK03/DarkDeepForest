...
RandomTree::RandomTree(const vector<vector<FeatureType>>& features,
  ...
  int numRandomPositions)
{
  ...
  while (curNode < SZ(m_nodes))
  {
	...
	// NOTE:　探索範囲強化のため、continueは廃止
        ...
        // END CUT HERE
	// NOTE: gini係数更新を変更する. randomでベストなginiにするようにするw
        if ( gini < bestGini)
        {
          bestGini = gini;
...
AnswerType RandomTree::estimate(const vector<FeatureType>& features){
    // ルートからたどるだけ
...
void RandomTree::setLeaf(TreeNode& node, int& curNode, const vector<AnswerType>& answers) {
    node.leaf = true;
